...
def mtFileClassification(inputFile, modelName, modelType, plotResults=False, gtFile=""):
    ...
    if (modelType == 'svm') or (modelType == 'svm_rbf'):
        ...
        [Classifier, MEAN, STD, classNames, mtWin, mtStep, stWin, stStep, computeBEAT] = aT.load_knn_model(modelName)
...
def speaker_diarization(filepath, num_of_speakers=0, mt_size=2.0, mt_step=0.2, st_win=0.05, lda_dim=35, plot_=False):
    ...
    :param mt_size:         (optional) mid-term window size
    :param mt_step:         (optional) mid-term window step
    :param st_win:          (optional) short-term window size
    :param lda_dim:         (optional) LDA dimension (0 for no LDA)
    :param plot_:           (optional) boolean flag for plotting
    ...
    gtFile = filepath.replace('.wav', '.segments');  # open for annotated file
