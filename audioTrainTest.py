...
def load_knn_model(knn_model_path, is_regression=False):
    try:
        fo = open(knn_model_path, "rb")
    except IOError:
...
def fileClassification(inputFile, modelName, modelType):
    ...
    if (modelType) == 'svm' or (modelType == 'svm_rbf'):
        ...
        [Classifier, MEAN, STD, classNames, mtWin, mtStep, stWin, stStep, computeBEAT] = load_knn_model(modelName)
    elif modelType == 'randomforest':
...
def fileRegression(inputFile, modelName, modelType):
    ...
    if modelType == 'svm':        
        ...
        [_, _, _, mtWin, mtStep, stWin, stStep, computeBEAT] = load_knn_model(regressionModels[0], True)
    ...
    for ir, r in enumerate(regressionModels):
        ...
        if modelType == 'svm':
            ...
            [Model, MEAN, STD, mtWin, mtStep, stWin, stStep, computeBEAT] = load_knn_model(r, True)
        curFV = (MidTermFeatures - MEAN) / STD                  # normalization
